...
CSV_PATH = Path("../satellite_data/orbital_elements/CryoSat-2.csv")
TARGET_COL = "mag_change"
...
def load_data(csv_path: Path) -> pd.DataFrame:
    ...
    for col in ['argument of perigee', 'mean anomaly', 'right ascension']:
        df[col + '_unwrap'] = np.unwrap(df[col])

    # Compute differences
    df['delta_arg'] = df['argument of perigee_unwrap'].diff()
    df['delta_anom'] = df['mean anomaly_unwrap'].diff()
    df['delta_ra']   = df['right ascension_unwrap'].diff()

    # Magnitude of change feature
    df['mag_change'] = np.sqrt(
        df['delta_arg']**2 + 
        df['delta_anom']**2 + 
        df['delta_ra']**2
    )
...
def evaluate_model(model, X_scaled, y_true, y_scaler):
    """Return predictions in original scale plus RMSE/MAE."""
...
def sliding_window_forecast(model, history, steps, x_scaler, y_scaler):
    ...
    for _ in range(steps):
        window_feats = np.array(history[-WINDOW_SIZE:])  # ensure size
        # Build feature vector [lag_WINDOW_SIZE .. lag_1]
...
def main():
    ...
    dump(x_scaler, "x_scaler_magchange.joblib")
    dump(y_scaler, "y_scaler_magchange.joblib")
    ...
    model.save_model("test_regr_crysat2_magchange.json")
